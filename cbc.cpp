...
vector<string> Encryption(string plain)
{
    ...
    for(int i = 0; i < IV.length(); i++) {
        bitset<4> bin(IV[i] >= 'A' ? IV[i] - 'A' + 10 : IV[i] - '0');
        previousCiphertext += bin.to_string();
    }
    ...
    for (int i = 0; i < blocks.size(); i++)
    {
        ...
        string xored = xorString(blocks[i], previousCiphertext);
        cout << "After XOR with " << (i == 0 ? "IV" : "previous cipher") << ": " << xored << "\n";

        string cipher = DES(xored); // Encrypt the XORed block
        cipherTexts.push_back(cipher);
        previousCiphertext = cipher; // Save for next block
...
string Decryption(vector<string> cipher)
{
    ...
    // Decrypt each block with CBC mode
    for (int i = 0; i < cipher.size(); i++)
    {
        ...
        string decrypt = DES(cipher[i]); // Decrypt the block

        // XOR with previous ciphertext (or IV for first block)
        string plaintext = xorString(decrypt, previousCiphertext);
        text.push_back(plaintext);

        // Save current ciphertext for next block
        previousCiphertext = cipher[i];

        cout << "Decrypted Block " << i + 1 << " (Binary): " << plaintext << "\n";
    }
